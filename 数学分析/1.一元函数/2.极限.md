...
- **$p$ 级数**：$\sum\limits^\infty_{n=1}\dfrac{1}{n^p}$，当 $p>1$ 时收敛
  - **证明（放缩成等比形式）**：
    ...
    - 设级数和为 $S_n$，则 $S_n-S_m \leq 2^k S_{2^{k+1}-2^k} < \sum\limits^{n-1}_{i=1} r^i < \dfrac{1}{1-r}$
...
- **压缩映射方法**：
  ...
  - **证明**：
    - L中值定理 + 三角不等式得 $|x_{n+p}-x_n| \leq \sum\limits^{n+p}_{k=n+1} r^{k-1}|x_1-x_0| \leq \\ |x_1-x_0|\dfrac{r^n}{1-r}$
    - 由柯西收敛原理即得 $x_n$ 收敛
