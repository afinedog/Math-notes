  ...
  - 分式极限：
    ...
    - $\dis\lim_{n\to\infty}\frac{2n^2-1}{3n^2+2} = \frac{2}{3}$
      - **证明**：
        ...
        - 只需变形即得 $n > \sqrt{\cfrac{7-6\e}{3(1-3\e)}}$ 即可
  - 指数极限
    - $|q^n| < \e$
    - $|\sqrt[n]{a} - 1| < \e$
      - **证明**：
    ...
    - $|\sqrt[n]{n}-1| < \e$
      ...
      - **证明2**：利用均值不等式 $$\sqrt[n]{n} = \sqrt[n]{\sqrt{n}\sqrt{n}\cdot 1\cdot 1\cdots} \leq \frac{\sqrt{n}+\sqrt{n}+1+1+\cdots}{n} = \frac{2}{\sqrt{n}} + 1-\frac{2}{n}$$
    - $|\sqrt[n]{n^p} - 1| < \e$（$p$ 是有限数）
      - **证明**：仿照上面方法即可
  - 根式极限
...
- **stolz($\dfrac{\infty}{\infty}$ 型)**：若 $y_n$ 是单增正无穷大量，则 $\lim\limits_{n\to\infty}\cfrac{x_n-x_{n-1}}{y_n-y_{n-1}} = a \red\Rt \lim\limits_{n\to\infty}\cfrac{x_n}{y_n} = a$
  - <font color='chartreuse'>左式的意思是收敛速度，极限为 $0$ 说明 $N$ 极大时 $x_n$ 的发散速度远快于 $y_n$，即 $x_n$ 远远大于 $y_n$</font>
  - **证明**：
...
- **stolz($\dfrac{0}{0}$ 型)**：若 $y_n\searrow 0$ 且 $x_n\to 0$，则 $\lim\limits_{n\to\infty}\cfrac{x_n-x_{n-1}}{y_n-y_{n-1}} = a \red\Rt \lim\limits_{n\to\infty}\cfrac{x_n}{y_n} = a$
  - <font color='red'>同上，我们只证明 $a=0$ 的情况即可</font>
  - <font color='chartreuse'>左式的意义是收敛速度，故极限为 $0$ 说明 $N$ 极大时 $x_n$ 的收敛速度远慢于 $y_n$，即 $x_n$ 早就收敛完成了
  ...
  - **证明（$x_n > 0$）**：
    - 取 $a = +\infty$，再对 $\dfrac{1}{x_n}$ 和 $\dfrac{1}{y_n}$ 使用上述累加法即可
  - **证明（其它情况）**：
    - 取绝对值数列 $|x_n|$，由上面的推导可得 $\dfrac{|x_n|}{y_n}\to 0$，显然它蕴含 $\dfrac{x_n}{y_n}\to 0$
  - **理解**：
- **stolz函数极限**：设 $T>0$ 为常数
  - 若
...
- **广义部分和极限**：
  ...
  - **解（级数证明法）**
    - 设 $b_n = p_n-p_{n-1}$
    - 仿照之前题目，发现原式等于 $$\frac{p_nA_n - (p_n-p_{n-1})A_{n-1} - (p_{n-1}-p_{n-2})A_{n-2}-...}{p_n} = \frac{\sum\limits^n_{k=1} b_kA_k + 有限余项}{p_n}$$
...
- **调和级数**：$\sum\limits^n_{i=1}\dfrac{1}{n} - \ln n\to  \g$
  ...
  - **证明（单调有界法）**：
    ...
    - **单调性**：
      ...
      - <font color='chartreuse'>用对数不等式可直接得到结论。但为了避免循环论证，还是要用初等方法</font>
...
- **不动点方法（单减）**：
  ...
  - <font color='chartreuse'>然后再判断 $F$ 是否满足单增情况的适配条件，从而应用不动点方法即可</font>
- **不动点方法**：条件弱化为 $|f'(x)| < 1$ 即可
  - **证明**：
    - 
  
#### 习题
  
- **1**：$$x_1 = 1，x_2 = \frac{1}{2}，x_{n+1} = \cfrac{1}{1+x_n}$$
  - **解**：
    - 显然 $f$ 单减，故 $F = f\circ f$ 单增
    - 再易得 $x_{2n}$ 和 $x_{2n-1}$ 在 $x^* = \dfrac{\sqrt{5}-1}{2}$ 均满足不动点方法条件，故极限为 $x^*$
    - <font color='chartreuse'>此时 $x_{2n}$ 在 $x^*$ 左端单增收敛，$x_{2n-1}$ 在 $x^*$ 右端单减收敛。通过直接计算就容易发现这一点</font>
- **2**：$$x_1 = 2，x_{n+1}  = 2+\cfrac{1}{x_n}$$
  - **解**：
...
- **六大定理的等价性**：
  ...
  - **确界存在定理**
    - **证明**：
      ...
      - <font color='chartreuse'>貌似跳过了凝聚定理和单调有界收敛定理</font>
  - **凝聚定理**：
    - **证明**：和之前相同，用闭区间套定理即可
  - **单调有界数列收敛定理**：
    - **闭区间套法**：
...
- **重要极限**：$\lim\limits_{x\to 0}\dfrac{\sin x}{x} = 1$
  ...
  - **证明2**：
    ...
    - 积分法：
      ...
      - 用定积分求出弧长 $l = \dis\int^1_x \sqrt{1+(\sqrt{1-t^2})'}dt$，从而写出 $\sin x$ 的定积分形式，由此计算得 $\sin x < x < \tan x$
  - **证明3**：
...
- **广义极限**
- **Cauchy收敛原理**：
  - **有限点处**：$\lim\limits_{x\to x_0}f(x)$ 存在且有限 $\LR \forall \e>0，\exist \d > 0，\forall |x_1-x_2| < \d$ 都有 $$|f(x_1)-f(x_2)| < \e$$
  - **无穷远处**：$\lim\limits_{x\to\infty}f(x)$ 存在且有限 $\LR \forall \e>0，\exist X>0，\forall x_1,x_2>X$ 都有 $$|f(x_1)-f(x_2)| < \e$$
  - **证明**：
    - **必要性**：定义即可
    - **充分性**：转化为数列 $\{f(x_n)\}$，然后应用数列的柯西收敛原理即可
