...
- **参数法比微分法更简单**：
  - $(y')^2-2xy'+1 = 0$
    - **解（微分法）**：微分得 $\cfrac{dp}{dx} = \cfrac{p}{p-x}$，是齐次方程，解得 $|px(2-px)| = Ce^{-x^2}$，然而它并没有给出 $x,y$ 的参数解，故实际上没做出来
    - **解（参数法）**：
      - 设 $p = tx$，则 $x = \cfrac{1}{\sqrt{t(2-t)}}，p = \sqrt{\cfrac{t}{2-t}}$
...
- **奇解**：设一阶ODE有特解 $\Gamma$。若对于 $\Gamma$ 上任意点 $Q$，都存在一个不同于 $\Gamma$ 的解与其在 $Q$ 点相切，则 $\Gamma$ 为方程的奇解
  - <font color='chartreuse'>奇解即处处不唯一的特解</font>
  - **实例**：克莱罗方程的解
- **奇解存在的必要条件**：
  ...
  - **本质**：隐函数不能唯一，所以隐函数存在定理的导数条件必须不成立
...
- **包络存在的必要条件**：
  ...
  - 包络是用 $\Big( x(C),y(C) \Big)$ 隐函数 + 相切性进行正证</font>
  - **理解**：
...
- **p判别法**：
  - 首先写出p判别式 $F_p = 0$，再和原方程 $F=0$ 联立，变形得 $\begin{cases} x = f(p) \\ y = g(p) \end{cases}$
    - <font color='red'>这里不能直接把 $F_p = 0$ 当成微分方程求解，只能用常规变形</font>
...
- **特解法**：
  - 因为奇解必定是特解，故求出全部特解，再验证是否满足不唯一性和相切性即可
