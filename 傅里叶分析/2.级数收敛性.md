...
- **（定理1.1）均方收敛定理**：在单位圆上可积的函数，其傅立叶级数平方收敛 $$\lim\limits_{N\to\infty}\frac{1}{2\pi} \int^{2\pi}_0 |f(\t)-S_N(f)(\t)|^2d\t = 0$$
...
- **希尔伯特空间**：满足正定性、完备性的内积空间
  - **实例**：
    - $\R$ 和 $\C$ 中取欧氏内积，是有限维Hilbert空间
    - $L^1$ 空间是无穷维Hilbert空间
- **准希尔伯特空间**：只满足正定性或完备性的内积空间
- **平方收敛空间 $\mathcal{R}$**：复单位圆上黎曼可积函数的集合
  - **Hermit内积**：$\dis (f,g) = \frac{1}{2\pi} \int^{2\pi}_0 f(\t)\overline{g(\t)}d\t$
  - **范数**：$\dis\|f\| = \sqrt{\frac{1}{2\pi}\int^{2\pi}_0 |f(\t)|^2d\t}$
  - **C-S不等式**：
    - **证明**：
      - 设 $A = \sqrt{\l}|f(\t)|，B = \dfrac{1}{\sqrt{\l}}|g(\t)|$
      - 对AB运用基本不等式后，两端积分得范数下的不等式
      - 最后取 $\l = \cfrac{\|g\|}{\|f\|}$，即得C-S不等式
  - **类Hilber性**：
...
- **证明均方收敛定理**：$\|f-S_N\|\to 0$
  ...
  - 若 $f$ 仅可积，则由可积逼近定理（见第一节），存在连续函数 $g$ 被控制在 $|f|$ 的上界内，从而 $\|f-g\|^2 \leq B\int |f-g|d\t \leq \varepsilon$
    - 再因为 $g$ 可被三角逼近，得到结果
...
- **Parseval恒等式**：$\sum\limits^\infty_{n=-\infty} |a_n  |^2 = \|f\|^2$
  ...
  - **推论**：
    ...
    - **（定理1.4）Riemann-Lebesgue引理**：圆上可积函数，Fourier系数收敛于0 
      - **等价命题**：圆上可积函数，$\int^{2\pi}_0 f(\t)sin(N\t)d\t \to 0$（$cos$ 同理）（实虚部分比较）
        - **理解**：否则永远有无穷多个有效项
...
- **（定理2.1）可微收敛定理**：圆上可积函数 $f$，在 $\t_0$ 处可微，则Fourier级数收敛
  - **证明**：设 $F(t) = \begin{cases} \frac{f(\t_0-t)-f(\t_0)}{t}，t\neq 0且|t|<\pi \\ -f'(\t_0)\quad\ ，t=0\end{cases}$
    - 由可微性，$F$ 在圆上可积
    - $S_N(f)(\t_0) - f(\t_0)$，选择Dirichlet核卷积意义，可将右项化为积分再合并，得 $\frac{1}{2\pi}\int^{2\pi}_0 F(t)tD_N(t)dt$（Fourier系数形式）
    - 拆分Dirichlet核，得被积函数连续性。由R-L引理，该积分收敛到0
  - **推论**：只需满足Lipschitz条件也可
- **（定理2.2）局部收敛定理**：若两个圆上可积函数，在 $\t_0$ 的邻域上相等，则它们的Fourier级数极限相等
...
- **N度的三角多项式**：$|n| > N$ 的项均为0（对称中心为0）
  - 构造 $P_N(\t) = e^{i(2N)\t}f_N(\t)$，$\tilde{P}_N(\t)$ 同理
...
- **对称破坏性证明**：
  ...
  - 设 $f(\t) = \sum\limits^\infty_{k=1} \alpha_k P_{N_k}(\t)$
  - $f(\t)$ 收敛到一个圆上连续函数
    ...
    - 从而 $m\to\infty$ 时无界，而此时 $S(f)(0)\to f(0)$，即在0上连续的函数 $f(\t)$，其Fourier级数不收敛
