...
- <font color='red'>双曲方程的中心差分格式的稳定条件又称为（CFL条件）
  - 波动方程中心差分格式 $r\leq 1$
  - 对流方程迎风格式 $|r|\leq 1$</font>
...
- **三点差分格式**：将泰勒展式代入波动方程得 $$\frac{u^{n+1}_j - 2u^n_j + u^{n-1}_j}{\tau^2} = a^2\frac{u^n_{j+1} - 2u^n_j + u^n_{j-1}}{h^2} + R^n_j(u) $$
...
- **恒稳定差分格式**：<font color='chartreuse'>首先用 $n-1,n,n+1$ 层的（中心差商的权平均）逼近，再找出最佳权重值，使得差分格式恒稳定</font>
  - 设 $D_n = \cfrac{u^n_{j+1} - 2u^n_j + u^n_{j-1}}{h^2}$，待定差分格式为 $$ \frac{u^{n+1}_j - 2u^n_j + u^{n-1}_j}{\tau^2} = a^2\Big[ \t D_{n+1} + (1-2\t)D_n + \t D_{n-1} \Big]  $$
  - 当 $\t = 0$ 时就是显格式
...
- **迎风格式**：$$ \begin{cases} \cfrac{u^{n+1}_j - u^n_j}{\tau} + a_j\cfrac{u^n_j - u^n_{j-1}}{h} = 0 & 若 a_j\geq 0 \\\\ \cfrac{u^{n+1}_j - u^n_j}{\tau} + a_j\cfrac{u^n_{j+1} - u^n_j}{h} = 0 & 若 a_j < 0 \end{cases}$$
  - **稳定条件（CFL条件）**：$\abs{\dfrac{\tau}{h}}\max\limits_{j} |a_j| \leq 1$
