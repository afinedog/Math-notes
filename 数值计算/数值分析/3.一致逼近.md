...
- **一致逼近性**：$\forall f\in C[0,1]$，都有 $\lim\limits_{n\to\infty} \|f-B_nf\|_{\infty} = 0$
  - **证明**：
    ...
    - 由Cantor定理，$f$ 在 $[0,1]$ 上一致连续，故对 $\e$ 存在一个相应的 $\d(\e)$
    - **拉普拉斯方法**：
...
- **Weierstrass一致逼近定理**：
  ...
  - **证明**：
    - 取线性变换 $x = (b-a)t + a$，设 $f(x)$ 变为 $\wt f(t)$
      - <font color='chartreuse'>即将定义域 $[a,b]$ 线性变换为 $[0,1]$</font>
    - 由Bernstein多项式的一致逼近性，存在多项式 $\wt p(t) = (B_nf)(t)$ 满足题设
    - 再将其换元回来即可

## 最佳一致逼近

- **偏差**：$\D (f,p_n) = \|f-p_n\|_{\infty}$
  - <font color='chartreuse'>多项式 $p_n$ 与 $f$ 相差最大的值</font>
- **最小偏差（最佳逼近）**：$E_n(f) = \inf\limits_{p_n\in\mc P_n} \D(f,p_n)$
  - <font color='chartreuse'>用 $n$ 次多项式逼近 $f$ 时，可以将最大距离压缩到的最小值</font>
  - **单调性**：$E_0(f) \geq E_1(f) \geq ...$
    - **证明**：
      - 显然多项式次数越高，其种类越多，即 $|\mc P_n|$ 越大，且是包含升链
      - 从而相差值只可能变小，不可能变大
- **存在性定理（Borel逼近定理）**：$\forall f\in C[a,b]，\exists p_n\in\mc P_n$ 使得 $\D(f,p_n) = E_n(f)$
  - <font color='chartreuse'>即最小偏差是可取到的下确界，从而存在最佳一致逼近多项式</font>
  - **证明**：
...
- **偏离点**：由于 $f,p_n$ 都是闭区间上的连续函数，故 $f-p_n$ 的上确界可取到。我们称它的最值点为偏离点
...
- **切比雪夫交错定理**：
  ...
  - <font color='chartreuse'>因为最佳逼近曲线一定位于 $f$ 曲线的中间位置，而交错点越多说明越居中</font>
  - **证明（充分性）**：
...
- **零次逼近**：取 $P_0(x) = \cfrac{\max f + \min f}{2}$ 即可
  - 此时交错点组为 $\min f，\max f，f(x_0) = P_0$

### 待定系数法（一次逼近）


- **待定系数法**：设 $P_1(x) = cx + d$，此时 $e(x) = P_1(x)-f(x)$ 的极值点就是交错点
  - <font color='chartreuse'>如果极值点数量少于 $n+2 = 3$，那么端点也是交错点</font>
- **拐点简化定理**：若 $f$ 在 $(a,b)$ 上无拐点，则求解下列方程即可 $$\begin{cases}  e'(x_0) = 0 \\ e(a) = -e(x_0) = e(b) \end{cases}$$
  - **证明**：
    - 此时 $P_1''(x) = 0$，再由无拐点即得 $e''(x)$ 不变号，从而 $e'$ 单调，即 $e$ 至多有 $1$ 个极值点
    - <font color='chartreuse'>此时在 $[a,b]$ 上 $e$ 最多有 $3$ 个交错点，即两端点 + 内部极值点</font>
    - <font color='red'>一般我们只讨论无拐点情况，否则需要解非线性方程，几乎无法手算</font>
- **例1**：求 $f(x) = \sin x$ 在 $[0,\pi]$ 上的一次最佳逼近
  - **解**：
    - 易得内部无拐点
    - 待定系数法得 $p(x) = \dfrac{1}{2}$，此时 $\begin{cases} p(0) - f(0) = \dfrac{1}{2} \\\\ p(\dfrac{\pi}{2}) - f(\dfrac{\pi}{2}) = -\dfrac{1}{2} \\\\ p(\pi) - f(\pi) = \dfrac{1}{2} \end{cases}$，即这三个点构成交错点组
    - 也就是说，当 $n=1$ 时，已经存在 $n+2 = 3$ 交错点组，故它就是最佳逼近
    - <font color='chartreuse'>即 $\mc P_1$ 和 $\mc P_0$ 上的最佳逼近是同一个</font>
- **例2**：求 $f(x) = \sin x$ 在 $[0,\dfrac{\pi}{2}]$ 上的一次最佳逼近
  - **解**：
    - 易得内部无拐点
    - 待定系数法得 $p(x) = \dfrac{2}{\pi}x + \dfrac{4-\pi}{2\pi}$，此时 $\begin{cases} p(0) - f(0) = \dfrac{4-\pi}{2\pi} > 0 \\\\ ... \\\\ p(\dfrac{\pi}{2}) - f(\dfrac{\pi}{2}) = \dfrac{4-\pi}{2\pi} > 0 \end{cases}$，故这三个点构成交错点组
- **例3**：求 $f(x) = x^3$ 在 $[-1,1]$ 上的一次最佳逼近
  - **解**：
    - 此时存在拐点
    - 发现 $f$ 是对称区间上的奇函数，故 $P_1$ 也是奇函数，可设为 $P_1(x) = ax$
    - 计算得极值点 $x = \pm\sqrt{\dfrac{a}{3}}$，此时只有两个内部极值，故必须令端点也是交错点
    - 解方程 $e(\sqrt{\dfrac{a}{3}}) = e(-1)$ 即得 $a = \dfrac{3}{4}$
...
- **Chebyshev多项式**：$T_n(x) = \cos(n\arccos x)$
  - **递推公式**：$T_{n+1}(x) = 2xT_n(x) - T_{n-1}(x)$
    - **证明**：
      - 由和差化积公式易得 $\cos(n+1)\t = 2\cos\t\cos n\t - \cos(n-1)\t$
      - 再代入即可
  - **多项式性**：$T_n(x)$ 是 $n$ 次多项式
    - **证明**：
      - 易得 $T_0(x) = 1，T_1(x) = x$
  ...
  - **最值点**：设 $x_k = \cos\dfrac{k}{n}\pi$，则 $T_n(x_k) = (-1)^{k-1}\sup\limits_{x\in [-1,1]} |T_n(x)|$
    - **证明**：代入得显然
  - **周期交错性**：在单位圆上，零点和最值点以 $\dfrac{1}{2n}\pi$ 为步长（再取余弦）交错排列
    - <font color='chartreuse'>奇数点是零点，偶数点是最值点</font>
- **切比雪夫零偏差定理**：$p_n(x) = \cfrac{1}{2^{n-1}}T_n(x)$ 是 $[-1,1]$ 上的最小零偏差多项式
  - <font color='chartreuse'>前面的系数是为了保证首一</font>
  - **证明**：
    - 转化为求 $0$ 的最佳一致逼近多项式
    - 由切比雪夫定理，要找交错点组 $-1\leq x_1<...<x_{n+1} \leq 1$ 使得 $p_n(x_i) = \sigma_i$
      - <font color='chartreuse'>其中 $\sigma_i = \pm \sup |p_n(x)|$，且 $\sigma_i\sigma_{i+1} < 0$</font>
    - 由切比雪夫多项式的性质，显然可选取 $\hkh{x_k = \cos\dfrac{k\pi}{n}}^{n}_{k=0}$ 作为交错点组，再令首项系数为 $1$ 即可
