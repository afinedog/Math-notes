...
- **变换阵**：数组空间中，线性变换 $\ms A$ 在自然基下的矩阵称为变换阵
  - **性质**：
    - 对任意向量，$\ms A(\a) = A\a$
    - 对任意一组基，$\ms A(\e_1,...,\e_n) = A(\e_1,...,\e_n)$
  - <font color='red'>我们所说的线性映射的矩阵一般都指自然基下的矩阵</font>
  - <font color='chartreuse'>本身没有什么特殊价值，但是做题比较好用</font>
...
- **推论（非极大无关组）**：若线性扩张定理中 $(\a_i)$ 线性无关，但不是极大线性无关组，则满足条件的线性映射 $\ms A$ 不唯一
...
- **线性映射空间 $L(U,V)$**：$U$ 到 $V$ 的全体线性映射组成的集合
  ...
  - 同构于 $F^{m\times n}$，映射为 $\p(\ms C_{ij}) = E_{ij}$<font color='chartreuse'>（对偶基在 $(\a_i),(\b_i)$ 下的矩阵）</font>
...
- **线性函数的对偶基举例**
  ...
  - **构造性证明**：
    - <font color='chartreuse'>很简单，直接凭感觉构造即可</font>
...
- **已知两坐标阵，求过渡矩阵**
  - **解**：
    ...
    - <font color='chartreuse'>也可以写成 $n^2$ 阶矩阵乘积，但是算起来太麻烦了。不如逐个算方程组</font>
<!-- - **像核转置定理**：后面再说吧 -->
- **伪逆**：
  - 设 $P$ 是竖条矩阵，列向量彼此线性无关
...
- **投影变换阵**：
  - 设
    ...
    - <font color='chartreuse'>显然 $A$ 和 $B$ 都是竖条矩阵
    - 实际上我们在说出基矩阵这个概念时，就已经把 $W$ 同构到数组空间来处理了</font>
    - $\p:W\to U$ 是投影变换
  - 则
    - 投影变换阵为 $(A,O)(A,B)^{-1}$
...
- **次幂等分解（不变子空间分解）**：设 $\p$ 是 $n$ 维线性空间 $V$ 的线性变换，则存在 $m\in\N^+$ 使得 $$\Im\p^m = \Im\p^{m+1}，\ker\p^m = \ker\p^{m+1}，V = \Im\p^m \oplus \ker\p^m$$
  - <font color='red'>即退化变换关于自复合运算收敛（此时 $V$ 被分解为两个 $\p$ 的不变子空间）
  - <font color='chartreuse'>因为像是不断收缩的，而收缩最终也得有个下界。在下界上收缩不动，就变为恒等</font>
