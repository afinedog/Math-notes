...
- **线性映射的矩阵（坐标写法）**：
  ...
  - **证明**：
    ...
    - 则 $\sigma(X_n) = Y_m$ 就是相应的数组空间线性映射，显然它和 $\ms A$ 是一一对应的
...
- **线性映射矩阵的唯一性**：两种写法中的 $A_{m\times n}$ 是相同的
  - **证明**：
...
- **推论（非极大无关组）**：若上面的 $(\a_i)$ 线性无关，但不是极大线性无关组，则满足条件的线性映射 $\ms A$ 不唯一
  ...
  - **例子**：
    - 设 $\alpha = x_1\alpha_1 + ... + x_n\alpha_n$，$\ms (\a) = \b = x_n\b_1+...+x_n\b_n$
    - 若取（错位映射）满足 $\mathscr{A}(\alpha_1) = \dfrac{x_2}{x_1}\beta_2，\mathscr{A}(\alpha_2) = \dfrac{x_3}{x_2}\beta_3，...，\mathscr{A}(\alpha_n) = \dfrac{x_1}{x_n}\beta_1$
      - 那么此时也有 $\ms A(\a) = \b$
      - 这样就出现了两个不同的线性映射，且 $\a$ 在它们下的像相同
    - 如果上面的 $\ms A$ 对每个向量、每一组基都满足类似关系，那么就会出现第二个满足线性扩张定理的映射，与唯一性矛盾
...
- **线性变换（知二求三）**：线性变换的矩阵、基的向量形式、（基的像）的向量形式。若知道其中两个就能求出全部三个
...
- **线性映射的乘法**：
  - **翻转性**：$(\mathscr{AB})\Big[ (\alpha_i)_nX_n \Big] = (\beta_i)_mABX_n$
...
- **多项式空间的坐标映射**
  ...
  - **多项式基（Kronecker基）**：设 $f_i = \prod\limits^n_{1\leq j\leq n，j\neq i}(x-a_j)$，则 $\{f_i\}$ 是一组基
    ...
    - **证明（取值映射法）**：
      ...
      - 此时 $\mathscr{A}(f_i) = f_i(a_i)\bold{e_i}$，像线性无关，故原像也线性无关（**核心**）
  - **两基的过渡矩阵**：
    - 设过渡关系为 $(1,x,...,x^{n-1}) = (f_1,...,f_n)P$
    - 两边作用（取值映射）得 $V(a_1,...,a_n) = \diag(f_i(a_i))P$（其中 $V$ 是范德蒙德矩阵），即 $P = \diag\Big( f_i(a_i) \Big)^{-1}\cdot V(a_1,...,a_n)$
...
- **线性函数的对偶基举例**
  ...
  - <font color='chartreuse'>前面已经知道，$U$ 上任意一组基，在对偶空间 $U^*$ 中对应一组对偶基。这里就是已知（单个对偶基向量）时求（可能的原基）</font>
  - **构造性证明**：
...
- **投影变换阵**：
  - 设
    ...
    - $A = (\e_1,...,\e_r)，B = (\e_{r+1},...,\e_n)$ 分别是 $U$ 和 $V$ 的某个基矩阵
    - <font color='chartreuse'>实际上我们在说出基矩阵这个概念时，就已经把 $W$ 同构到数组空间来处理了</font>
    - $\p:W\to U$ 是投影变换
  - 则
    - 投影变换阵为 $(A,O)(A,B)^{-1}$
  ...
  - **证明（正交投影阵）**：
    ...
    - 易得此时 $(A^TA)^{-1}A^T$ 是 $A$ 的左逆，是 $B$ 的左核，另一种情况同理
