...
- <font color='red'>虽然双线性映射比平衡映射多了一些条件，但它并没有获得更多的基本性质，只是计算更加方便而已</font>
...
- **（推论5.3）张量积的函子性**：
  ...
  - <font color='red'>张量积是函子，将（右R模范畴和左R模范畴的积）对应到（张量积范畴）
  - 这里就是它对态射的作用</font>
  - **证明**：
...
- **（定理5.5）张量左模**：
  - 设
    - $R,S$ 是环，$A$ 是S-R双模，$B$ 是左R模
    - $f:A\to A'$ 是双模同态，$g:B\to B'$ 是左模同态
  - 则
    - $A\otimes_R B$ 是左S模，且满足 $s(a\otimes b) = (sa)\otimes b$
    - $f\otimes g:A\otimes B\to A'\otimes B'$ 是左S模同态
    - <font color='red'>张量模和态射模的结构完全不同
      - $R$ 作用到 $A$ 右边，作用到 $B$ 左边，故 $A,B$ 可用 $R$ 上的张量积连接起来
      - $S$ 作用到 $A$ 左边，从而也作用到 $A\otimes B$ 左边</font>
  - **证明**：定义即可
  - **本质**：<font color='red'>$S$ 在 $A\otimes_R B$ 上的作用本质就是张量分析中的缩并</font>
- **（定理5.5）张量右模**：
  - 设
    - $R,S$ 是环，$C$ 是右R模，$D$ 是R-S双模
    - $h:C\to C'$ 是右模同态，$k:D\to D'$ 是双模同态
  - 则
    - $C\otimes_R D$ 是右S模，且满足 $(c\otimes d)s = c\otimes ds$
    - $h\otimes k:C\otimes D\to C'\otimes D'$ 是右S模同态
  - **证明**：同上
- **（定理5.5）张量双模**：
  - 设
    - $R,S$ 是环，$M$ 是R-S双模，$N$ 是S-T双模
    - $\p:M\to M'$ 和 $\psi:N\to N'$ 都是双模同态
  - 则
    - $M\otimes_S N$ 是R-T双模，且满足 $r(m\otimes n)t = (rm)\otimes (nt)$
    - $\p\otimes\psi:M\otimes N\to M'\otimes N'$ 是双模同态
  - **证明**：同上
- **（定理5.5）张量积的双模性**：$R$ 上的张量积是R-R双模
  - **证明**：由张量双模直得

### 基本性质

- **（定理5.7）张量积自反性**：
  - 若 $R$ 是含幺环，$A$ 是右R模，$B$ 是左R模
  - 则存在R模同构 $A\otimes R \cong A，R\otimes B\cong B$
  - **证明（B）**：
    - **模性**：
      - 显然 $R$ 是R-R双模
      - 易得 $R\otimes B$ 是左R模
    - **平衡性**：易得 $R\times B\to B，(r,b)\mapsto rb$ 是平衡映射
    - **构造同构**：
      - 易得 $\a:R\otimes B\to B，r\otimes b\mapsto rb$ 是群同态和左R模同态
      - 易得 $\b:B\to R\otimes B，b\mapsto 1_R\otimes b$ 是群同态和左R模同态
      - 易得 $\a\b = 1_B，\b\a = 1_{R\otimes B}$，即互逆
  - **本质**：<font color='red'>不难发现它跟前面对偶模的"向量直线同构"是类似的。后面我们会知道，张量积和态射类是彼此伴随的函子</font>
- **（定理5.8）张量积结合律**：
...
- **张量积交换条件**：
  - 设 $R$ 是含幺交换环
  - 则任意R模张量积都有 $A\otimes B \cong B\otimes A$
  - **证明**：
    - 设 $f:A\times B\to B\otimes A，(a,b)\mapsto n\otimes m$
    - **平衡性**：易得
    - **构造**：由典范分解定理，存在唯一的群同态 $\ol f$ 满足 $f = \ol f i$
    - **可逆性**：反向操作得到 $\ol g$，易得它和 $\ol f$ 互逆
- **（定理5.9）直积传递性**：
  - 设若 $R$ 是环，$A,A_i$ 是右R模，$B,B_j$ 是左R模
  - 则有以下R模同构 $$\dkh{\sum\limits_{i\in I}A_i}\otimes B \cong \sum\limits_{i\in I} (A_i\otimes B)$$ $$A\otimes \dkh{\sum\limits_{j\in J} B_j} \cong \sum\limits_{j\in J} (A\otimes B_j)$$
  - **证明**：简单构造同构即可
  - **推论**：
    - 若 $A,A_i$ 都是S-R双模，则第一行是左S模同构
    - 若 $B,B_j$ 都是R-S双模，则第一行是右R模同构
- **（定理5.11）单自由传递性**：
...
- **（推论5.12）双自由传递性**：
  ...
  - 则 $A\otimes B$ 是自由R-R双模，基为 $W = \{x\otimes y\}$，基数为 $|X||Y|$
  - <font color='red'>张量分析中的标准基定理</font>
  - **证明**：
- **（推论5.13）子环自由传递性**：
  - 设 $S$ 是含幺环，$R$ 是其含幺子环
  - 若 $F$ 是自由左R模，基为 $X$
...
- **（命题5.4）左张量函子的右正合性**：
  ...
  - <font color='red'>张量函子 $D\otimes \cdot$ 是右正合函子</font>
  - **证明**：仿照对偶模即可
  - **反例（逆命题不成立）**：
    - 设 $R = \Z$
  ...
  - **本质**：<font color='red'>Ten函子和Hom函子是伴随关系，Hom函子都保左正合，Ten函子都保右正合</font>
- **（命题5.4）右张量函子的右正合性**：
  - 左R模正合列 $$ A\xto f B\xto g C\to 0 $$
  - 则任取右R模 $D$，下面都是阿贝尔群正合列 $$A\otimes D\xto{f\otimes 1_D} B\otimes D\xto{g\otimes 1_D} C\times D\to 0$$
  - <font color='red'>张量函子 $\cdot\otimes D$ 也是右正合函子
  - 注意这两个张量函子都是协变函子，因为张量积的对称性更强（在交换环上左右可交换）
  - 代价是，Ten函子的对称性会比Hom函子差一些。为此我们发明了平坦模的概念</font>
  - **证明**：同上

### 张量函子的短正合条件

- **平坦模**：
  - 设 $D$ 是左R模，若张量函子 $\cdot\otimes D$ 是正合函子，则 $D$ 称为平坦模
  - 设 $D$ 是右R模，若张量函子 $D\otimes \cdot$ 是正合函子，则 $D$ 称为平坦模
- **直和传递性**：$\sum\limits_{i\in I}M_i$ 是平坦模 $\LR \forall M_i$ 是平坦模
  - **证明**：
- **投射平坦定理**：投射模是平坦模
  - <font color='red'>平坦模保Ten函子的正合性，投射模和内射模保Hom函子的正合性
  - 投射模的结构非常完美，但内射模就复杂得多。这种不对称性本质是因为"构造"和"分解"的复杂度不同
    - 张量函子是提升（构造），而Hom函子是投影（分解）
    - 左伴随是自由（构造），而右伴随是遗忘（分解）
    - 积分是恢复信息（构造），而微分是消灭信息（分解）。积分比微分复杂得多</font>
  - **证明**：
...
- **（定理5.10）伴随结合律**：
  ...
  - <font color='red'>即张量积函子 $\cdot\otimes_R\cdot$ 和态射类函子 $\Hom_R(\cdot,\cdot)$ 彼此伴随</font>
  - **证明**：

### 张量积的对偶空间

- **张量积的对偶模**：$(A\otimes B)^* \cong A^*\otimes B^*$，指派法则 $(f\otimes g)(a\otimes b) = f(a)\otimes g(b)$
  - <font color='red'>张量分析中的升降指标公式</font>
  - **证明**：不难发现它就是上面的张量积自然变换性
- **对偶运算律**：
  - 设 $f,\p$ 是左模同态，$g,\psi$ 是右模同态
  - 则 $(f\otimes g)(\p\otimes \psi) = (f\p)\otimes (g\psi)$
  - **证明**：