...
- **类似章节**：
  ...
  - 泛函分析的对偶空间
- **背景**：
  - 态射类和张量积是由已知正合列构造新正合列的常用方法（它们是经典的伴随函子）
- **符号约定**：
...
- **诱导同态**：
  ...
  - <font color='red'>诱导同态本质都是函子在态射上的作用（函子不仅将对象对应起来，也将态射也对应起来）
    - 群同态 $\p$ 的诱导同态 $\ol\p:G\to G/\ker \p$，本质是群范畴到商群范畴的函子在 $\p$ 上的作用
    - 模同态 $\p$ 的诱导同态 $\ol\p:\Hom(D,A)\mapsto \Hom(D,B)$，本质是模范畴到阿贝尔群范畴的函子在 $\p$ 上的作用
    - 基本群的诱导同态 $h_*:\pi_1(X,x_0)\to \pi_1(Y,y_0)$，本质是带基点拓扑范畴到群范畴的函子在连续映射 $h$ 上的作用</font>
- **诱导同态复合运算法则**：
...
- **R-S双模**：$A$ 既是左R模，又是右S模，且满足结合律 $r(as) = (ra)s$
  - **实例**：
    ...
    - 交换环 $R$ 的模都是R-R双模
- **双模范畴 $R$-$\Mod$-$S$**
  - <font color='red'>态射集必须同时是左模和右模同态，即 $\Hom_{R-S}(A,B)$</font>
  - **反例（只是左模同态，不是右模同态）**：
    - 设 $R$ 是非交换环，则右平移作用 $f:R\to R，m\mapsto mx$ 是左模同态，但不是右模同态
...
- **（定理4.8）态射双模**：
    ...
    - <font color='red'>当且仅当 $R = S$ 时，$\Hom_R(A,B)$ 是R-R双模</font>
    - **证明**：显然
